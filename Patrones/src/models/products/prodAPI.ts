...
import { ProductsFactoryDAO } from './prodFactory';
import { ProductsDAO } from './prodFactory';
import { ProductI, ProductsDTO } from './prodInterface';
import { prodJoiSchema } from './prodSchema';
...
export default class ProductsAPI {
  ...
  static async getInstance(): Promise<ProductsAPI> {
    if (!this.instance) {
        infoLogger.info('Inicializando api de productos');
  ...
  validateSchema(data: any) {
    const result: ValidationResult = prodJoiSchema.validate(data);

...
export const UsersModel = mongoose.model(dbCollection, userSchema)
...
class Users {
    ...
    async updateUser(id: any, user: UserObject) {
        try{
            const response = await this.users.findByIdAndUpdate(id, user, { new: true })
            return response
        } catch (error:any) {
            errorLogger.error(error.message)
    ...
    async deleteUser(id: string) {
        try{
            const response = await this.users.findByIdAndDelete(id)
            return response
        } catch (error:any) {
            errorLogger.error(error.message)
            throw new Error(`Error updating user: ${error.message}`)
